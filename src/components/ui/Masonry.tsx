...
const useMedia = (queries: string[], values: number[], defaultValue: number): number => {
  ...
  const get = () =>
    isClient ? values[queries.findIndex(q => window.matchMedia(q).matches)] ?? defaultValue : defaultValue;
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full">
      {grid.map(item => (
        ...
        >
          <div
            className="relative w-full h-full bg-cover bg-center rounded-[16px] shadow-[0px_10px_50px_-10px_rgba(0,0,0,0.2)] uppercase text-[10px] leading-[10px]"
